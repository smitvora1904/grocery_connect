        ...
        <div class="page-container">
            ...
            <div class="main-content">
                <div class="col-lg-9" style="margin-top: 10px; width: 800px">
                                <h2 class="title-1 m-b-25">Pending Payments</h2>
                                ...
                                <?php
                                   echo "<table class='table table-borderless table-striped table-earning' style='width:1175px'>";
                                        ...
                                        $sql = "select cust_id,next_due_date,current_due_amt from credit_customers where current_due_amt > 0 and seller_id=".$_SESSION['seller_id'];
                                        $result = mysqli_query($conn,$sql);
                                        while ($row=mysqli_fetch_array($result)) {
                                            $sql2 = "select name,contact,address from customer_details where cust_id=".$row['cust_id'];
                                            $result2 = mysqli_query($conn,$sql2);
                                            $row2=mysqli_fetch_array($result2);

                                            /*$sql3 = "select order_id from orders_log where cust_id=".$row['cust_id']." and seller_id=".$_SESSION['seller_id']." and status = 1";
                                            ...
                                            $date2=date_create($row['next_due_date']);
                                            $diff=date_diff($date1,$date2);
                                            if($diff->format("%a") > 30)
                                            {
                                                echo "<td style='color: red;'>".$row['next_due_date']."</td>";    
                                            }
                                            else
                                            {
                                                echo "<td>".$row['next_due_date']."</td>";
                                            }
                                            echo "<td>".$row['current_due_amt']."</td>";
                                            echo "<td>";
                                            echo "<a style='text-align: center;height: 25px;width: 50px;background-color: green;color: white;' href='./accountClear.php?id=".$row['cust_id']."'>  Paid </a>";
                                            //echo "</form>";
                                            echo "</td>";
                                            echo "</tr>";
                                        }
