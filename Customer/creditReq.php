...
<?php
    ...
    if (isset($_POST['resign'])) {
    	$sql = "delete from credit_customers where cust_id=".$_COOKIE['user'];
    	mysqli_query($conn,$sql);
    	header('location: kitchen.php');
    }
    if (isset($_POST['place'])) {

    	$sql = "update customer_details set credit_req=".$_COOKIE['current_seller']." where cust_id=".$_COOKIE['user'];
    	mysqli_query($conn,$sql);
    	header('location: kitchen.php');    	
    }
	...
	<div class="product" style="padding-top: 10px;padding-bottom: 10px;">
		<div class="container">
			<div class="spec " align="left">
				<h3 id="heading" align="left" style="color: #f19210;">Please Note</h3>
				</div>
				<div class=" con-w3l" align="left" style="margin-top: 10px;">
					<p style="font-size: 20px;color: grey;">
            	<ul style="color: grey;">
            		<li>Being a Credit Customer of a seller will give you credit period of 30 days on orders upto Rs.5000 from a seller.</li>
            		<li>Right now, the mode of payment after credit period is cash and you need to visit store personally and pay.</li>
            	</ul>
            </p><br><br>
            <?php
        	...
        	if(is_array($row))
        	{
        		...
        		$sql2 = "select shop_name from seller_details where seller_id=".$row['seller_id'];
    			$result2 = mysqli_query($conn,$sql2);
        		$row2 = mysqli_fetch_array($result2);
        		echo "<p style='font-size: 24px;color: red;''>
						You will Resign as Credit Customer from ".$row2['shop_name']."
				</p>";
            	
            	if($row['current_due_amt'] == 0)
            	{
				echo "<form method='post'>
    			<div class='input-group'>
    			<h2 class='t-button'>
				<button name='resign'><span class='label label-danger' style='float: left;margin-right: 30px;'>Resign</span></button>
			  </h2>
    			</div>
    			</form>";
    			}
    			...
    			{
    				echo "<p style='font-size: 24px;color: red;'>
						You can not Resign. You have Rs.".$row['current_due_amt']." DUE for seller, ".$row2['shop_name']."
					</p>";
    			}
        	...
        	else
        	{
        		...
        		$sql = "select credit_req from customer_details where cust_id=".$_COOKIE['user'];
    			$result = mysqli_query($conn,$sql);
        		$row = mysqli_fetch_array($result);
        		if($row['credit_req'] == 0)
        		{
    			...
    			<div class='input-group'>
    			<h2 class='t-button'>
				<button name='place'><span class='label label-success' style='float: left;margin-right: 30px;'>Place Request</span></button>
			  </h2>
    			</div>
    			</form>";
    			}
    			...
    			{
    				$sql = "select shop_name from seller_details where seller_id=".$row['credit_req'];
    				$result = mysqli_query($conn,$sql);
        			$row = mysqli_fetch_array($result);

        			echo "<p style='font-size: 20px;color: red;'>
        			You already a Request of Credit Customer at ".$row['shop_name']."
        			</p>";
    			}
        	}
        		?>
    			...
    			else
    			{
        			...
        			echo "<p style='font-size: 20px;color: red;'>
        			You are already a Request of Credit Customer at ".$row['shop_name']."
        			</p>";
    			}*/
    			?>
				</div>
